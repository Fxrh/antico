...
Dockicon::Dockicon(Frame *frame, Systray *sys_tr, QWidget *parent) : QWidget(parent)
{
    ...
    setToolTip(title);
    frm->frame_state = 1; // 1 = raised, 0 = unmapped;
}
...
{
    // get style path
    QSettings *antico = new QSettings("/home/rafal/antico/antico.cfg", QSettings::IniFormat, this);
...
void Dockicon::mousePressEvent(QMouseEvent *event)
{
    ...
    {
        if (frm->frame_state==0/*frame_state == 0*/)
        {
            frm->frame_state = 1;
	    frm->set_active();
            frm->raise_it();
        ...
        if (frm->frame_state==1/*frame_state == 1*/)
        {
            frm->frame_state = 0;
	    frm->set_inactive();
            frm->unmap_it();
