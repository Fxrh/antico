...
palette.setColor( backgroundRole(), QColor( 0, 0, 0 ) ); //   palette.setColor( backgroundRole(), QColor( 40, 216, 248 ) );
...
void Frame::read_settings()
{
    ...
    kill_pix = stl_path + style->value("kill_pix").toString();
    minimalize_pix = stl_path + style->value("minimalize_pix").toString();
...
/*
void Frame::unmap_it(QMouseEvent *event)
{
  frame_state = 0;
  unmap_it();
...
void Frame::create_borders()
{
    ...
    QGridLayout * layout_min_max_resize_close = new QGridLayout();
    layout_min_max_resize_close->setMargin(0);
    layout_min_max_resize_close->setSpacing(0);
    layout->addLayout(layout_min_max_resize_close,0,2);    
    ...
    layout_min_max_resize_close->addWidget(tl_bdr, 0, 1);  //    layout->addWidget(tl_bdr, 0, 0);
    

    // minimalize button
    minimalize_button = new Border(this);
    minimalize_button->setToolTip(tr("Minimize(L)"));
    minimalize_button->setFixedSize(top_bdr_height, top_bdr_height);
    minimalize_button->setPixmap(minimalize_pix);
    minimalize_button->setScaledContents(true);
    minimalize_button->setAlignment(Qt::AlignLeft);
    layout_min_max_resize_close->addWidget(minimalize_button, 0, 0);  //    layout->addWidget(tl_bdr, 0, 0);    
    ...
    tr_bdr->setAlignment(Qt::AlignRight);//    tr_bdr->setAlignment(Qt::AlignCenter);
    layout_min_max_resize_close->addWidget(tr_bdr, 0, 2);
    
    kill_button = new Border(this);
    kill_button->setToolTip(tr("Kill Aplication"));
    kill_button->setFixedSize(top_bdr_height, top_bdr_height);
    kill_button->setPixmap(kill_pix);
    kill_button->setScaledContents(true);
    kill_button->setAlignment(Qt::AlignCenter);   
    layout->addWidget(kill_button, 0, 0);  //layout->addWidget(tm_bdr, 0, 1);    
    ...
    tm_bdr->set_pixmap(QPixmap(header_active_pix), QPixmap(header_inactive_pix), title_color);
    tm_bdr->setFixedHeight(top_bdr_height);
    layout->addWidget(tm_bdr, 0, 1);  //layout->addWidget(tm_bdr, 0, 1);
    ...
    // top left (icon)
    connect(tl_bdr, SIGNAL(mouse_left_press()), this, SLOT(maximize_it()));//  connect(tl_bdr, SIGNAL(mouse_left_press()), this, SLOT(iconify_it()));
    
    
    connect(tl_bdr, SIGNAL(mouse_right_press()), this, SLOT(iconify_it()));//   connect(tl_bdr, SIGNAL(mouse_right_press()), this, SLOT(maximize_it()));
    // top right (icon)
    connect(tr_bdr, SIGNAL(mouse_left_press()), this, SLOT(destroy_it()));
    
    
    
    connect(minimalize_button, SIGNAL(mouse_left_press()), this, SLOT(minimalize()));    
    connect(kill_button, SIGNAL(mouse_left_press()), this, SLOT(kill_it()));     
    
    // top mid (title bar)
    connect(tm_bdr, SIGNAL(mouse_double_click()), this, SLOT(maximize_it()));//    connect(tm_bdr, SIGNAL(mouse_double_click()), this, SLOT(iconify_it()));
...
/*
  ...
  //  connect(this, SIGNAL(mouseMoveEvent(QMouseEvent *event)), this, SLOT(move_top_mid(QMouseEvent *)));
}
...
void Frame::mouseMoveEvent(QMouseEvent *event)
{
    ...
    {
        move_top_mid(event);    //emit mouse_move(event);
    }
}*/
...
//hide();
   // repaint(p.x(),p.y(),frame_w,frame_h);
    move(p.x(), p.y());
//show();
